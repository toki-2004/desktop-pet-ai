...
12. **位置/大小记忆与一键召回**：退出时自动记住桌宠位置与大小，下次启动原样
    ...
    桌宠掉到屏幕外无法操作。
13. **余额查询（多平台）**：设置页"余额"页绑定 DeepSeek / Kimi / 硅基流动
    账号后定时轮询余额；余额文本常驻桌宠左上角（可拖动、滚轮调字号、右键
    菜单开关），余额增减有浮动动画。
14. **工作状态标签 + AI 感知**：以余额升降判定工作状态（降低=工作中，
    不变=空闲中，升高=充值了），标签显示在时段标签右侧；工作状态同时作为
    AI 的 prompt 参数，状态切换会触发 AI 说话（各自冷却，避免刷屏）。
