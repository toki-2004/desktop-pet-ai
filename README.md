...
2. **厂商预设**：设置页提供预设下拉——DeepSeekWeb2API（本机
   `http://127.0.0.1:8000/v1`）/ 硅基流动 / Kimi / 自定义；选预设自动填
   base_url 与模型，只需填 API Key（本机服务可免）。
3. **聊天输入框**：桌宠下方常驻可输入文本框（可拖动、滚轮调字号、位置记忆），
   回车即与 AI 对话。
4. **摸头反应**：单击桌宠播放一次摸头动画并触发 AI 反应；长按 = 拖动，不触发。
5. **自言自语（情境触发）**：沿袭原版触发逻辑——时段窗口（早/午/下午/晚/深夜，
...
9. **外观**：导入 PNG/GIF 更换皮肤；摸头动画（GIF，只播一遍、保留透明背景）
   与窗口置顶可在设置切换。
...
- 一个 OpenAI 兼容的对话服务（本机 DeepSeekWeb2API 或任意云端 Key）
