...
7. **摸头语录**：每次单击随机弹出一条摸头语录（与动画同步，一次点击一条）；
   ...
   语录支持 [high] / [low] 前缀，会优先显示与当前好感档位匹配的回应。
8. **自言自语（情境触发）**：语录按内容自动分类，用不同方式触发——时点问候
   ...
   为空时回退到这里）；工作状态与好感档位切换也有专属触发：
   [work_down]（余额降低=工作中）/ [work_flat]（余额不变=空闲中）/ [work_up]
   （余额升高=充值了），以及 [affection_high] / [affection_mid] / [affection_low]
   （好感档位切换时触发，带冷却防刷屏）。天气接口免 key
   （ip-api.com 定位 + Open-Meteo）。
   文本库、开关、间隔均可在设置中调整。
9. **好感度系统**：摸头/单击互动好感上升（默认每次 +5），长时间不互动
   随时间下降（默认每 5 分钟 -1，退出期间同样衰减）；好感分高/中/低三档
   （默认 ≥80 高、<40 低），档位切换触发对应自言自语并影响摸头语录选择；
   桌宠旁显示"好感"小标签（可关），设置"好感/状态"页可调整初始值、
   上限、增减速度、阈值与冷却。
10. **工作状态显示**：以余额升降为判定标准——余额降低=正在工作（烧余额）、
    余额不变=没在工作、余额升高=充值了；桌宠下方"工作中/空闲中/充值了"
    标签常态显示，并触发对应的工作状态语录；余额下降后保持"工作中"3 分钟
    （设置可调，默认 180 秒），避免余额同步延迟导致很快又显示"空闲中"。
11. **语录库 JSON 化**：摸头与自言自语语录均改为独立的 JSON 文件存储
   （`pet_head_quotes.json` / `self_talk_quotes.json`），首次运行自动生成预置默认
   语录库；旧版 `self_talk.txt` 会自动迁移为 JSON（原文件保留）。
12. **窗口置顶开关**：设置中可统一控制"显示在最上层"，通知、桌宠、余额文本、
    浮动字四种窗口同步切换，不会出现"通知置顶但余额文本不置顶"的情况。
13. **余额显示美化**：余额常驻文本移至图片右上方，白底黑字，字号可调。
14. **设置热生效**：修改任何设置（账号、外观、通知、自言自语、置顶、刷新频率等）后立即生效，
   无需重启。
15. **基础交互**：左键拖动桌宠，位置自动记忆；右键呼出菜单。
16. **开机自启**：右键菜单可开关"开机自启"（写入当前用户 Run 键，无需管理员，
    开机后用 pythonw 静默启动）。
17. **时段状态显示**：桌宠正下方常态显示当前是高峰时段（红）还是空闲时段（绿），
    纯色边框 + 半透明底纹 + 圆角矩形；互动动画播放期间固定在常态位置不动。
18. **系统托盘图标**：左键单击托盘图标显示/隐藏桌宠，右键菜单可打开通知设置、
    外观设置、显示/隐藏桌宠或退出；桌宠找不到时也能从容退出。
19. **健壮性**：余额查询带 20 秒看门狗（DNS 卡死等网络异常不再无声卡死，自动重试）；
    配置文件原子写入并自动保留备份，损坏时不再静默丢失设置。
20. **字号统一**：通知气泡、余额文本、浮动文字共用同一字号（默认 14pt，范围 8-30），
